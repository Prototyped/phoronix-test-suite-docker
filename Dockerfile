...
RUN set -eu; \
    ...
    apt -y autoremove; \
    apt -y autoclean; \
    apt -y install tzdata locales-all aptitude curl htop tini unzip; \
    ln -sf /etc/zoneinfo/Etc/UTC /etc/localtime; \
    apt -y install locales; \
    update-locale LANG=en_GB.UTF-8; \
    aptitude -y install php-cli php-xml; \
    apt -y build-dep aircrack-ng; \
    curl --fail-with-body -LSso /tmp/phoronix-test-suite_10.8.4_all.deb https://phoronix-test-suite.com/releases/repo/pts.debian/files/phoronix-test-suite_10.8.4_all.deb; \
    dpkg -i /tmp/phoronix-test-suite_10.8.4_all.deb; \
    apt -y clean; \
